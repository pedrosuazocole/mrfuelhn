...
async function migrarAV2() {
  ...
  try {
    ...
    // ── MIGRACIÓN: estacion_id en mantenimiento_categorias ────────────────
    // Permite asignar una categoría de mantenimiento a una estación específica.
    // NULL = aplica a todas las estaciones (comportamiento anterior)
    try {
      await runAsync(`ALTER TABLE mantenimiento_categorias ADD COLUMN estacion_id INTEGER REFERENCES estaciones(id) ON DELETE SET NULL`);
      console.log('✅ Columna estacion_id agregada a mantenimiento_categorias');
    } catch (e) {
      console.log('  ℹ️  estacion_id ya existe en mantenimiento_categorias');
    }
    ...
    // ═══════════════════════════════════════════════════════════════════
    // CATEGORÍAS DE MANTENIMIENTO ESPECÍFICAS — UNO CIRCUNVALACIÓN
    // ═══════════════════════════════════════════════════════════════════
    try {
      const estCircunvalacion = await getAsync(
        `SELECT id FROM estaciones WHERE LOWER(nombre) LIKE '%circunvalac%' LIMIT 1`
      );

      if (!estCircunvalacion) {
        console.log('  ⚠️  Estación "Uno Circunvalación" no encontrada — se omiten sus categorías de mantenimiento.');
      } else {
        const uc = estCircunvalacion.id;

        // Helper para crear categoría + ítems de forma idempotente
        async function crearCategoriaMantenimientoUC(nombre, descripcion, orden, items) {
          ...
          console.log(`✅ Categoría "${nombre}" (Uno Circunvalación) creada con ${items.length} ítems`);
        }

        // ── 1. Generador-Compresor Semanal ────────────────────────────────
        await crearCategoriaMantenimientoUC(
          'Mantenimiento Generador-Compresor Semanal — Uno Circunvalación',
          'Mantenimiento semanal de generador y compresor de aire — Uno Circunvalación',
          20,
          [
        ...
        await crearCategoriaMantenimientoUC(
          'Mantenimiento Generador Anual y Compresor Bimestral — Uno Circunvalación',
          'Mantenimiento anual de generador y bimestral de compresor de aire — Uno Circunvalación',
          21,
          [
        ...
        await crearCategoriaMantenimientoUC(
          'Mantenimiento Filtros y Sensores Dispensador Semestral — Uno Circunvalación',
          'Inspección semestral de filtros de combustible, sensores y pantallas del dispensador — Uno Circunvalación',
          22,
          [
        ...
        await crearCategoriaMantenimientoUC(
          'Mantenimiento Mangueras-Boquillas-Calibración-Generador Trimestral — Uno Circunvalación',
          'Revisión trimestral de mangueras, boquillas, calibración de dispensador y generador — Uno Circunvalación',
          23,
          [
        ...
        await crearCategoriaMantenimientoUC(
          'Mantenimiento Extintores-Tanque de Agua-Generador Bimestral — Uno Circunvalación',
          'Mantenimiento bimestral de extintores, tanque de presión de agua y generador — Uno Circunvalación',
          24,
          [
            ...
            { nombre: 'Extintor Lubricentro — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor del Lubricentro' },
            { nombre: 'Extintor Tienda — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Tienda' },
            { nombre: 'Extintor Cocina — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Cocina' },
            { nombre: 'Fecha de Vencimiento de Extintores', desc: 'Registrar fechas de vencimiento de todos los extintores (Bombas 1-4, Lubricentro, Tienda, Cocina)' },
      ...
      console.log('  ⚠️  Error creando categorías de mantenimiento de Uno Circunvalación:', e.message);
    }

    // ═══════════════════════════════════════════════════════════════════
    // CATEGORÍAS DE MANTENIMIENTO ESPECÍFICAS — BUENOS AIRES
    // ═══════════════════════════════════════════════════════════════════
    try {
      const estBuenosAires = await getAsync(
        `SELECT id FROM estaciones WHERE LOWER(nombre) LIKE '%buenos aires%' LIMIT 1`
      );

      if (!estBuenosAires) {
        console.log('  ⚠️  Estación "Buenos Aires" no encontrada — se omiten sus categorías de mantenimiento.');
      } else {
        const ba = estBuenosAires.id;

        // Helper para crear categoría + ítems de forma idempotente
        async function crearCategoriaMantenimientoBA(nombre, descripcion, orden, items) {
          const existe = await getAsync('SELECT id FROM mantenimiento_categorias WHERE nombre = ?', [nombre]);
          if (existe) {
            console.log(`  ℹ️  Categoría "${nombre}" ya existe, omitiendo`);
            return;
          ...
          console.log(`✅ Categoría "${nombre}" (Buenos Aires) creada con ${items.length} ítems`);
        }

        // ── 1. Generador-Compresor Semanal ────────────────────────────────
        await crearCategoriaMantenimientoBA(
          'Mantenimiento Generador-Compresor Semanal — Buenos Aires',
          'Mantenimiento semanal de generador y compresor de aire — Buenos Aires',
          25,
          [
            { nombre: 'Generador — Prueba de Operación (20 min bajo carga)', desc: 'Enciende el generador bajo carga y verifica que funcione correctamente durante al menos 20 minutos' },
            { nombre: 'Generador — Ruidos y Vibraciones', desc: 'Escucha ruidos inusuales y observa posibles vibraciones anormales' },
        ...
        // ── 2. Generador-Compresor Anual ───────────────────────────────────
        await crearCategoriaMantenimientoBA(
          'Mantenimiento Generador-Compresor Anual — Buenos Aires',
          'Mantenimiento anual de generador y compresor de aire — Buenos Aires',
          26,
          [
            { nombre: 'Generador — Limpieza Integral', desc: 'Limpia a fondo el generador, incluyendo componentes internos y externos' },
            { nombre: 'Generador — Calibración', desc: 'Ajusta los reguladores de voltaje y frecuencia si es necesario' },
        ...
        // ── 3. Filtros, Sensores Dispensador y Cisterna de Agua Semestral ──
        await crearCategoriaMantenimientoBA(
          'Mantenimiento Filtros-Sensores Dispensador-Cisterna Semestral — Buenos Aires',
          'Inspección semestral de filtros, sensores, pantallas del dispensador y limpieza de cisterna de agua — Buenos Aires',
          27,
          [
            { nombre: 'Filtro de Combustible — Bomba 1 (Tipo e Inspección)', desc: 'Inspeccionar y registrar el tipo de filtro de la Bomba 1' },
            { nombre: 'Filtro de Combustible — Bomba 2 (Tipo e Inspección)', desc: 'Inspeccionar y registrar el tipo de filtro de la Bomba 2' },
            ...
            { nombre: 'Filtro de Combustible — Bomba 6 (Tipo e Inspección)', desc: 'Inspeccionar y registrar el tipo de filtro de la Bomba 6' },
            { nombre: 'Filtro de Combustible — Bomba 7 (Tipo e Inspección)', desc: 'Inspeccionar y registrar el tipo de filtro de la Bomba 7' },
            { nombre: 'Sensor Dispensador — Bomba 1 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 1' },
            { nombre: 'Sensor Dispensador — Bomba 2 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 2' },
            { nombre: 'Sensor Dispensador — Bomba 3 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 3' },
            { nombre: 'Sensor Dispensador — Bomba 4 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 4' },
            { nombre: 'Sensor Dispensador — Bomba 5 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 5' },
            { nombre: 'Sensor Dispensador — Bomba 6 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 6' },
            { nombre: 'Sensor Dispensador — Bomba 7 (Bueno/Malo)', desc: 'Verificar estado del sensor de la Bomba 7' },
            { nombre: 'Pantalla Dispensador — Bomba 1 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 1' },
            { nombre: 'Pantalla Dispensador — Bomba 2 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 2' },
            { nombre: 'Pantalla Dispensador — Bomba 3 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 3' },
            { nombre: 'Pantalla Dispensador — Bomba 4 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 4' },
            { nombre: 'Pantalla Dispensador — Bomba 5 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 5' },
            { nombre: 'Pantalla Dispensador — Bomba 6 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 6' },
            { nombre: 'Pantalla Dispensador — Bomba 7 (Bueno/Malo)', desc: 'Verificar estado de la pantalla de la Bomba 7' },
            { nombre: 'Cisterna — Cierre de Válvula de Entrada de Agua', desc: 'Se cerró la válvula de entrada de agua' },
            { nombre: 'Cisterna — Vaciado Completo', desc: 'Se vació completamente la cisterna' },
            { nombre: 'Cisterna — Retiro de Residuos Sólidos', desc: 'Se retiraron residuos sólidos' },
            { nombre: 'Cisterna — Limpieza de Paredes y Fondo', desc: 'Se limpiaron paredes y fondo' },
            { nombre: 'Cisterna — Aplicación de Desinfectante', desc: 'Se aplicó desinfectante (cloro, etc.)' },
            { nombre: 'Cisterna — Enjuague con Agua Limpia', desc: 'Se realizó enjuague con agua limpia' },
            { nombre: 'Cisterna — Inspección del Estado General', desc: 'Se inspeccionó el estado de la cisterna' },
            { nombre: 'Cisterna — Revisión de Válvulas y Tuberías', desc: 'Se revisaron válvulas y tuberías' },
            { nombre: 'Cisterna — Llenado con Agua Potable', desc: 'Se llenó la cisterna con agua potable' },
            { nombre: 'Cisterna — Verificación de Funcionamiento', desc: 'Se verificó el correcto funcionamiento' },
          ]
        );

        // ── 4. Mangueras-Boquillas-Calibración-Generador-Trampa de Grasa Trimestral ──
        await crearCategoriaMantenimientoBA(
          'Mantenimiento Mangueras-Boquillas-Calibración-Generador-Trampa de Grasa Trimestral — Buenos Aires',
          'Revisión trimestral de mangueras, boquillas, calibración de dispensador, generador y trampa de grasa — Buenos Aires',
          28,
          [
            { nombre: 'Manguera Dispensador — Bomba 1 (Bueno/Malo)', desc: 'Verificar estado de la manguera de la Bomba 1' },
            { nombre: 'Manguera Dispensador — Bomba 2 (Bueno/Malo)', desc: 'Verificar estado de la manguera de la Bomba 2' },
            ...
            { nombre: 'Manguera Dispensador — Bomba 6 (Bueno/Malo)', desc: 'Verificar estado de la manguera de la Bomba 6' },
            { nombre: 'Manguera Dispensador — Bomba 7 (Bueno/Malo)', desc: 'Verificar estado de la manguera de la Bomba 7' },
            { nombre: 'Boquilla Dispensador — Bomba 1 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 1' },
            { nombre: 'Boquilla Dispensador — Bomba 2 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 2' },
            { nombre: 'Boquilla Dispensador — Bomba 3 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 3' },
            { nombre: 'Boquilla Dispensador — Bomba 4 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 4' },
            { nombre: 'Boquilla Dispensador — Bomba 5 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 5' },
            { nombre: 'Boquilla Dispensador — Bomba 6 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 6' },
            { nombre: 'Boquilla Dispensador — Bomba 7 (Bueno/Malo)', desc: 'Verificar estado de la boquilla de la Bomba 7' },
            { nombre: 'Calibración Dispensador — Bomba 1 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 1' },
            { nombre: 'Calibración Dispensador — Bomba 2 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 2' },
            { nombre: 'Calibración Dispensador — Bomba 3 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 3' },
            { nombre: 'Calibración Dispensador — Bomba 4 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 4' },
            { nombre: 'Calibración Dispensador — Bomba 5 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 5' },
            { nombre: 'Calibración Dispensador — Bomba 6 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 6' },
            { nombre: 'Calibración Dispensador — Bomba 7 (Resultado)', desc: 'Registrar resultado de calibración de la Bomba 7' },
            { nombre: 'Generador — Sistema de Refrigeración', desc: 'Revisa y limpia el radiador, y verifica el nivel y la calidad del refrigerante' },
            { nombre: 'Generador — Sistema Eléctrico', desc: 'Inspecciona los cables, conexiones y terminales en busca de desgaste o corrosión' },
            { nombre: 'Generador — Prueba de Carga Completa', desc: 'Opera el generador a su capacidad máxima para asegurarte de que puede soportar la carga' },
            { nombre: 'Trampa de Grasa — Retiro de Grasas y Residuos Sólidos', desc: 'Retiro de grasas y residuos sólidos' },
            { nombre: 'Trampa de Grasa — Limpieza Interna', desc: 'Limpieza interna (paredes y fondo)' },
            { nombre: 'Trampa de Grasa — Revisión de Tuberías', desc: 'Revisión de tuberías de entrada y salida' },
            { nombre: 'Trampa de Grasa — Inspección de Filtros y Rejillas', desc: 'Inspección de filtros y rejillas' },
            { nombre: 'Trampa de Grasa — Tratamiento Enzimático', desc: 'Aplicación de tratamiento enzimático (si aplica)' },
          ]
        );

        // ── 5. Extintores-Tanque de Agua-Generador Bimestral ──────────────
        await crearCategoriaMantenimientoBA(
          'Mantenimiento Extintores-Tanque de Agua-Generador Bimestral — Buenos Aires',
          'Mantenimiento bimestral de extintores, tanque de presión de agua y generador — Buenos Aires',
          29,
          [
            { nombre: 'Extintor Bomba 1 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Bomba 1' },
            { nombre: 'Extintor Bomba 2 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Bomba 2' },
      ...
      console.log('  ⚠️  Error creando categorías de mantenimiento de Buenos Aires:', e.message);
    }

    // ═══════════════════════════════════════════════════════════════════
    // CATEGORÍAS DE MANTENIMIENTO ESPECÍFICAS — POLVORÍN
    // ═══════════════════════════════════════════════════════════════════
    try {
      const estPolvorin = await getAsync(
        `SELECT id FROM estaciones WHERE LOWER(nombre) LIKE '%polvorin%' OR LOWER(nombre) LIKE '%polvorín%' LIMIT 1`
      );

      if (!estPolvorin) {
        console.log('  ⚠️  Estación "Polvorín" no encontrada — se omiten sus categorías de mantenimiento.');
      } else {
        const pv = estPolvorin.id;

        // Helper para crear categoría + ítems de forma idempotente
        async function crearCategoriaMantenimientoPV(nombre, descripcion, orden, items) {
          const existe = await getAsync('SELECT id FROM mantenimiento_categorias WHERE nombre = ?', [nombre]);
          if (existe) {
            console.log(`  ℹ️  Categoría "${nombre}" ya existe, omitiendo`);
            return;
          ...
          console.log(`✅ Categoría "${nombre}" (Polvorín) creada con ${items.length} ítems`);
        }

        // ── 1. Generador-Compresor Semanal ────────────────────────────────
        await crearCategoriaMantenimientoPV(
          'Mantenimiento Generador-Compresor Semanal — Polvorín',
          'Mantenimiento semanal de generador y compresor de aire — Polvorín',
          30,
          [
            { nombre: 'Generador — Prueba de Operación (20 min bajo carga)', desc: 'Enciende el generador bajo carga y verifica que funcione correctamente durante al menos 20 minutos' },
            { nombre: 'Generador — Ruidos y Vibraciones', desc: 'Escucha ruidos inusuales y observa posibles vibraciones anormales' },
        ...
        // ── 2. Generador Anual / Compresor Bimestral ──────────────────────
        await crearCategoriaMantenimientoPV(
          'Mantenimiento Generador Anual y Compresor Bimestral — Polvorín',
          'Mantenimiento anual de generador y bimestral de compresor de aire — Polvorín',
          31,
          [
            { nombre: 'Generador — Limpieza Integral', desc: 'Limpia a fondo el generador, incluyendo componentes internos y externos' },
            { nombre: 'Generador — Calibración', desc: 'Ajusta los reguladores de voltaje y frecuencia si es necesario' },
        ...
        // ── 3. Filtros y Sensores Dispensador Semestral ───────────────────
        await crearCategoriaMantenimientoPV(
          'Mantenimiento Filtros y Sensores Dispensador Semestral — Polvorín',
          'Inspección semestral de filtros de combustible, sensores y pantallas del dispensador — Polvorín',
          32,
          [
            { nombre: 'Filtro de Combustible — Bomba 1 (Tipo e Inspección)', desc: 'Inspeccionar y registrar el tipo de filtro de la Bomba 1' },
            { nombre: 'Filtro de Combustible — Bomba 2 (Tipo e Inspección)', desc: 'Inspeccionar y registrar el tipo de filtro de la Bomba 2' },
        ...
        // ── 4. Mangueras-Boquillas-Calibración-Generador Trimestral ──────
        await crearCategoriaMantenimientoPV(
          'Mantenimiento Mangueras-Boquillas-Calibración-Generador Trimestral — Polvorín',
          'Revisión trimestral de mangueras, boquillas, calibración de dispensador y generador — Polvorín',
          33,
          [
            { nombre: 'Manguera Dispensador — Bomba 1 (Bueno/Malo)', desc: 'Verificar estado de la manguera de la Bomba 1' },
            { nombre: 'Manguera Dispensador — Bomba 2 (Bueno/Malo)', desc: 'Verificar estado de la manguera de la Bomba 2' },
        ...
        // ── 5. Extintores-Tanque de Agua-Compresor-Generador Bimestral ───
        await crearCategoriaMantenimientoPV(
          'Mantenimiento Extintores-Tanque de Agua-Compresor-Generador Bimestral — Polvorín',
          'Mantenimiento bimestral de extintores, tanque de presión de agua, compresor de aire y generador — Polvorín',
          34,
          [
            { nombre: 'Extintor Bomba 1 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Bomba 1' },
            { nombre: 'Extintor Bomba 2 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Bomba 2' },
            { nombre: 'Extintor Bomba 3 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Bomba 3' },
            { nombre: 'Extintor Bomba 4 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de Bomba 4' },
            { nombre: 'Extintor Área de Descarga 1 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor del Área de Descarga 1' },
            { nombre: 'Extintor Área de Descarga 2 — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor del Área de Descarga 2' },
            { nombre: 'Extintor Oficina — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de la Oficina' },
            { nombre: 'Extintor Oficina 2da Planta — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de la Oficina 2da Planta' },
            { nombre: 'Extintor Generador — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor del Generador' },
            { nombre: 'Extintor Parte de Atrás Locales — Estado (Bueno/Malo)', desc: 'Verificar estado del extintor de la Parte de Atrás de los Locales' },
            { nombre: 'Fecha de Vencimiento de Extintores', desc: 'Registrar fechas de vencimiento de todos los extintores (Bombas 1-4, Área de Descarga 1 y 2, Oficina, Oficina 2da Planta, Generador, Parte de Atrás Locales)' },
            { nombre: 'Tanque de Presión — Verificación de Presión Interna', desc: 'Usa un manómetro para medir la presión del aire dentro del tanque y ajusta si es necesario' },
            { nombre: 'Tanque de Presión — Inspección de Válvulas y Conexiones', desc: 'Verifica que las válvulas de entrada y salida funcionen correctamente y las conexiones no estén obstruidas' },
            { nombre: 'Tanque de Presión — Limpieza Externa', desc: 'Limpia el tanque y el área circundante para evitar acumulación de polvo, suciedad o residuos' },
            { nombre: 'Tanque de Presión — Calibración del Sistema', desc: 'Asegúrate de que la bomba y el interruptor de presión estén ajustados según las necesidades del sistema' },
            { nombre: 'Compresor de Aire — Drenaje del Tanque de Aire', desc: 'Drena el agua acumulada en el tanque para evitar corrosión interna (con el compresor apagado y despresurizado)' },
            { nombre: 'Compresor de Aire — Mangueras y Conexiones', desc: 'Inspecciona mangueras y conexiones en busca de desgaste, grietas o fugas. Reemplaza componentes si es necesario' },
            { nombre: 'Generador — Inspección General', desc: 'Revisa el estado del alternador, correas, ventiladores y el sistema de escape' },
            { nombre: 'Generador — Prueba de Batería', desc: 'Mide el voltaje y carga de la batería. Sustituye si los niveles están fuera de rango' },
            { nombre: 'Generador — Revisión del Sistema de Combustible', desc: 'Comprueba el tanque de combustible, líneas y filtros en busca de obstrucciones o contaminantes' },
            { nombre: 'Generador — Drenaje del Agua del Tanque de Combustible', desc: 'Elimina cualquier acumulación de agua o sedimentos en el tanque' },
